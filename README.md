...
* 회원 가입 후 비밀번호 DB 저장 시 암호화 처리 ( bcryptPasswordEncoder 사용 )
* 로그인 & 로그아웃 처리, 회원 접근 권한 확인 ( spring-security 사용 )
* 게시판 글& 댓글 쓰기, 수정, 삭제 Interceptor를 이용해 접근 권한 확인
* 요청 받은 데이터 유효성 검사 처리( Validator 인터페이스와 Bean Validation 두 가지 사용 )
* 클라이언트 응답 요청만을 위한 @RestController 구현 ( RESTful API )
* @Transactional 이용한 트랜잭션 처리
* 핵심 기능 외의 공통 기능인, 메소드 진입 확인 로그출력과 성능측정을 AOP를 이용해 처리
* 회원 & 게시판 , 게시판 댓글 Mybatis 사용 CRUD 작업처리, 게시판 & 댓글 페이징 처리, 아이디 기억하기 ( 쿠키 )
* 회원가입시 이메일 인증 처리 ( JavaMailSenderImpl 사용 ), 파일 업로드 처리 ( multipartResolver 사용 )
* security 태그 라이브러리, 메시지 설정 파일( messageSource ), log4j2 사용
...
- HTTP 프로토콜의 이용, 기본적으로 무료 OPEN SSL 사용해서라도 HTTPS 프로토콜로 통신 가능하게 개선 필요
