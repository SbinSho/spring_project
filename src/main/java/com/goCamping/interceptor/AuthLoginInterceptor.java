...
public class AuthLoginInterceptor extends HandlerInterceptorAdapter{
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler)
		...
		// 객체에 저장되어 있는 아이디
		String session_userID = authInfo != null ? authInfo.getId() : "";
		// 요청 받은 아이디
		String request_userID = request.getParameter("user_id");
		
		// 현재 세션에 저장되어있는 loginUser가 존재하지 않으면
		// 로그인이 되어있지 않는 상태임으로 메인 홈페이지로 이동
		if( session_userID == null || !(request_userID.equals(session_userID))) {
			
			pre_out.println("<script>alert('잘못된 접근입니다.'); location.href='" + request.getContextPath() +"/';</script>");
			pre_out.flush();
			return false;
		} 
